...
//  WithPost tells the query-builder to eager-loads the nodes that are connected to
// the "post" edge. The optional arguments used to configure the query builder of the edge.
...
func (cq *CommentQuery) sqlAll(ctx context.Context) ([]*Comment, error) {
	...
	if query := cq.withAuthor; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].user_comments; fk != nil {
				ids = append(ids, *fk)
